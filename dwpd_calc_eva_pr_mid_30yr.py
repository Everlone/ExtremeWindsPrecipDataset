...
Repeat it for the mid period of the future.
...
# Process this after the main run
# DS_Name = 'CLMcom_MOHC', 'ETH_MOHC',  # amx is zero in all cases, gpdfit fails as exs = []
...
for dsi in range(len(DS_Name)):
    FileName = 'pr5d_fut_' + Period_Name[0] + '_' + DS_Name[dsi] + '.dpkl'
    ...
    FileName = 'pr5d_fut_' + Period_Name[period] + '_' + DS_Name[dsi] + '.dpkl'
    ...
    for xx in range(a):
        ...
        for yy in range(b):
            ...
            # Lines below added to deal with CLMcom_MOHC and ETH_MOHC mid and far
            # which both had 0 rain in all years at some locations, so no exceedances
            if not vecexs.any():
                GEV_R2[xx,yy] = np.nan
                GEV_R5[xx,yy] = np.nan
                GEV_R10[xx,yy] = np.nan
                GEV_R20[xx,yy] = np.nan
                GEV_R30[xx,yy] = np.nan
                GEV_R50[xx,yy] = np.nan
                GEV_R70[xx,yy] = np.nan
                GEV_R100[xx,yy] = np.nan
                GEV_R200[xx,yy] = np.nan
                GPD_R2[xx,yy] = np.nan
                GPD_R5[xx,yy] = np.nan
                GPD_R10[xx,yy] = np.nan
                GPD_R20[xx,yy] = np.nan
                GPD_R30[xx,yy] = np.nan
                GPD_R50[xx,yy] = np.nan
                GPD_R70[xx,yy] = np.nan
                GPD_R100[xx,yy] = np.nan
                GPD_R200[xx,yy] = np.nan
                continue
    ...
    fnout = # Path removed in online shared codes for security + DS_Name[dsi] + '_pr5d_' + Period_Name[period] + '_gpd_maps.dpkl'
